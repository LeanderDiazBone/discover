...
"""
class ArmPushObstacle(ArmEnvs):
    def __init__(self, backend="mjx", num_env=1, **kwargs):
        self.num_env = num_env
        super().__init__(backend=backend)

    def _get_xml_path(self):
        return f"envs/assets/panda_push_hard_obstacle_{self.num_env}.xml"
    ...
    def _get_initial_state(self, rng):
        ...
        cube_q_xy = jnp.array([-0.35, 0.6])+ self.cube_noise_scale * jax.random.uniform(subkey1, [2], minval=-1) #self.sys.init_q[:2] 
    ...
    def _get_initial_goal(self, pipeline_state: base.State, rng):
        ...
        cube_goal_pos = jnp.array([0.35, 0.6, 0.03]) + jnp.array([self.goal_noise_scale, self.goal_noise_scale, 0]) * jax.random.uniform(subkey, [3], minval=-1) # Push Far
