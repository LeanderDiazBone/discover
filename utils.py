...
from envs.high_dimensional_maze import HighDimensionalMaze
...
from envs.manipulation.arm_lift_hard import ArmLiftHard
from envs.manipulation.arm_push_obstacle import ArmPushObstacle
...
def create_parser():
    ...
    parser.add_argument("--max_replay_size", type=int, default=25000, help="Maximum size of replay buffer")
    parser.add_argument("--min_replay_size", type=int, default=1000, help="Minimum size of replay buffer")
    parser.add_argument("--num_evals", type=int, default=250, help="Total number of evaluations")
    ...
    parser.add_argument('--use_her', default=True, action="store_true", help="Whether to use HER for SAC")
    ...
    # Added arguments
    parser.add_argument('--goal_selection_strategy', type=str, default="HER", help="Goal selection strategy (e.g. HER, UNIFORM MEGA, UCB).")
    parser.add_argument('--ensemble_size', type=int, default=2, help="Number of critics used in ensemble used to compute standard deviation for the UCB goal selection strategy.")
    parser.add_argument('--ucb_mean_ini', type=float, default=1.0, help="Multiplicative factor for mean of value from initial state to goal.")
    parser.add_argument('--ucb_mean_tar', type=float, default=1.0, help="Multiplicative factor for mean of value from goal to target goal.")
    parser.add_argument('--ucb_std_ini', type=float, default=1.0, help="Multiplicative factor for std of value from initial state to goal.")
    parser.add_argument('--ucb_std_tar', type=float, default=1.0, help="Multiplicative factor for std of value from goal to target goal..")
    parser.add_argument('--filter_goals', type=str, default="none", help="Goal filtering strategy.")
    parser.add_argument('--negative_rewards', default=False, action="store_true", help="Whether to use negative or positive rewards.")
    parser.add_argument('--num_goals', type=int, default=50000, help="Number of goals sampled in each episode for selection.")
    parser.add_argument('--mega_cutoff_step_size', type=int, default=1, help="Step size for adjusting the mega_cutoff when using the MEGA goal filtering strategy.")
    parser.add_argument('--manipulation', default=False, action="store_true", help="Whether the environment is a manipulation task.")
    parser.add_argument('--tau', type=float, default=0.005, help="Poliak Averaging for target network.")
    parser.add_argument('--tau_policy', type=float, default=0.005, help="Poliak Averaging for target network policy.")
    parser.add_argument('--deterministic_eval', default=False, action="store_true", help="Whether the policy should be deterministic for evaluation.")
    parser.add_argument('--policy_delay', type=int, default=2, help="Delayed update of policy.")  
    parser.add_argument('--activate_final', default=False, action="store_true", help="Whether the final layer of the critic should be activated through a softplus function.")
    parser.add_argument('--her_relabeling_strategy', type=str, default="standard", help="Type of relabeling applied to trajectories.")
    parser.add_argument('--relabel_prob', type=float, default=0.7, help="Relable probability when using her future relabeling strategy.")
    parser.add_argument('--target_computation', type=str, default="min_random", help="Type of target critic value aggregation.")
    parser.add_argument('--transform_ucb', default=False, action="store_true", help="Whether to apply ucb transformation.")
    parser.add_argument('--standardize_ucb', default=False, action="store_true", help="Whether to apply ucb standization.")
    parser.add_argument('--relabel_prob_future', type=float, default=0.7, help="Probability of using uniform future relabeling.")
    parser.add_argument('--relabel_prob_uniform', type=float, default=0.0, help="Probability of using uniform relabeling.")
    parser.add_argument('--relabel_prob_geometric', type=float, default=0.0, help="Probability of using geometric future relabeling.")
    parser.add_argument('--critic_prior_file_name', type=str, default=None, help="File for loading parameters of prior critic used for guiding goal selection.")
    parser.add_argument('--obs_size_prior', type=int, default=None, help="Observation size prior critic.")
    parser.add_argument('--action_size_prior', type=int, default=None, help="Action size prior critic.")
    #parser.add_argument('--save_models', type=bool, default=None, help="Used for model checkpointing.")
    parser.add_argument('--save_models', default=False, action="store_true", help="Whether to save actor and critic models.")
    parser.add_argument('--use_metric', default=False, action="store_true", help="Whether to use learned metric_network for value function computation.")
    parser.add_argument('--eval_target_policy', default=False, action="store_true", help="Perform evaluation on target policy instead of original one.")
    parser.add_argument('--env_file_name', type=str, default="", help="File for loading maze layout of high dimensional maze.")
    parser.add_argument('--adaptation_strategy', type=str, default="", help="How to adapt UCB parameters.")
    parser.add_argument('--goal_log_delay', type=int, default=1, help="Delayed logging of goal table.")
    parser.add_argument("--render_delay", type=int, default=1, help="Delayed rendering of evaluation.")  
    parser.add_argument('--continue_strategy', type=str, default="uniform", help="Strategy for action selection after goal achievement.")
    parser.add_argument("--grid_size", type=int, default=0, help="Size of grid used for logging different quantities of ucb objective.")
    parser.add_argument("--adaptation_rate", type=int, default=100, help="Adaptation Rate.")  
    parser.add_argument('--goal_achievement_target', type=float, default=0.5, help="Goal achievement target for adaptation strategy.")
    parser.add_argument("--dir_var", type=float, default=0.0, help="Can be used to add normal noise to direction estimate.")
    parser.add_argument("--cur_alpha", type=float, default=0.0, help="Can be to introduce curiosity bonus during training.")
    parser.add_argument("--exploration_noise", type=float, default=0.4, help="Undirected exploration noise added to actions during exploration.")
    parser.add_argument("--algo", type=str, default="TD3", help="Type of algoritm used.")
    return parser

def create_env(env_name: str, backend: str = None, file_name: str = "", **kwargs) -> object:
    """
    ...
    elif env_name == "high_dimensional_maze":
        env = HighDimensionalMaze(backend=backend or "spring", file_name=file_name) # or "mjx")
    elif env_name == "ant":
        env = Ant(backend=backend or "spring") # or "mjx")
    ...
    elif env_name == "arm_lift_hard":
        env = ArmLiftHard(backend=backend or "mjx")
    elif env_name.startswith("arm_push_obstacle"):
        env = ArmPushObstacle(backend=backend or "mjx", num_env = int(env_name[-1]))
...
def create_eval_env(args: argparse.Namespace, file_name: str="") -> object:
    """
    ...
    return create_env(**vars(eval_arg), file_name=file_name)
...
def get_env_config(args: argparse.Namespace):
    ...
    legal_envs = ["reacher", "cheetah", "pusher_easy", "pusher_hard", "pusher_reacher", "pusher2",
                  ...
                  "arm_push_easy", "arm_push_hard", "arm_lift_hard", "arm_binpick_easy", "arm_binpick_hard", "arm_push_obstacle"] + [f"arm_push_obstacle_{i}" for i in range(10)]
...
class MetricsRecorder:
    ...
    """
    def __init__(self, num_timesteps: int, metrics_to_collect: List[str], exp_dir, exp_name, render_delay = 1):
        self.x_data = []
        self.y_data = {}
        self.y_data_err = {}
